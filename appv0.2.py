...
def analyze_style(text):
    ...
    # Removed "sample_text": text from the return dictionary
    return {
...
def generate_system_prompt(style, weights, input_lines, sample_text, qual_feedback=None):
    """Generate a system prompt with a random chunk as style reference."""
    prompt = f"""
    ...
    Here is a sample paragraph that exemplifies the style: "{sample_text}"

    Instructions:
    - Rewrite the input text to reflect the above style characteristics.
    - Use the sample paragraph as a reference for the writing style, but do not copy any phrases or sentences from it.
    - Ensure that the rewritten text is a paraphrase of the input text, maintaining its original meaning and factual content.
    - Do not include any information or phrasing from the sample paragraph in your rewrite.
...
def llm_qualitative_evaluation(original_input, reference, generated, orig_style, gen_style):
    ...
    try:
        ...
        if response.startswith("COPYING DETECTED"):
            ...
            score = (content_score + style_score) / 2
...
def main():
    ...
    st.write("Optimizes style to match reference text using random chunks while preserving input text content.")
        ...
        if st.button("Analyze URL") and url:
            with st.spinner("Fetching and analyzing text..."):
                ...
                if text:
                    ...
                    st.session_state.reference_chunks = [p.strip() for p in text.split('\n\n') if p.strip()]
    ...
    if uploaded_file and st.session_state.style and st.session_state.reference_chunks:
        input_text = uploaded_file.read().decode("utf-8")
        st.session_state.input_text = input_text
        st.session_state.best_text = input_text
        ...
        if st.button("Optimize Style"):
            with st.spinner("Optimizing with SPSA..."):
                ...
                # Optimization loop with random chunks
                for iteration in range(1, max_iterations + 1):
                    st.write(f"**Iteration {iteration}**")
                    # Select a random chunk for this iteration
                    chunk = random.choice(st.session_state.reference_chunks)
                    ...
                    prompt_plus = generate_system_prompt(st.session_state.style, w_plus, input_lines, chunk, feedback)
                    ...
                    prompt_minus = generate_system_prompt(st.session_state.style, w_minus, input_lines, chunk, feedback)
                    ...
                    prompt_current = generate_system_prompt(st.session_state.style, weights, input_lines, chunk, feedback)
